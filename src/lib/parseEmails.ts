...
function extractAmount(text: string): number | undefined {
  // 住所の検出（〒マーク、住所・住 所・住　所など）
  const hasAddress = /〒|住\s*所/.test(text);

  if (hasAddress) {
    const addressPatterns = [
      /合\s*計[^\d]*[¥￥]?\s*([0-9,]+)\s*円?/,
      /ご\s*請\s*求\s*金\s*額[^\d]*[¥￥]?\s*([0-9,]+)/,
      /お\s*支\s*払\s*い?\s*金\s*額[^\d]*[¥￥]?\s*([0-9,]+)/,
      /支\s*払[^\d]*[¥￥]?\s*([0-9,]+)/,
      /総\s*合\s*計[^\d]*[¥￥]?\s*([0-9,]+)/,
      /お\s*買\s*い\s*上\s*げ\s*金\s*額[^\d]*[¥￥]?\s*([0-9,]+)/,
      /決\s*済\s*金\s*額[^\d]*[¥￥]?\s*([0-9,]+)/,
      /請\s*求\s*金\s*額[^\d]*[¥￥]?\s*([0-9,]+)/,
    ];
...
export function parseEmails(emails: any[]): ParsedEmail[] {
  return emails.map((email) => {
    ...
    // 広告・マーケティングメールを除外
    const isAd =
      lowerText.includes("今だけ") ||
      lowerText.includes("セール") ||
      lowerText.includes("キャンペーン") ||
      lowerText.includes("お得な") ||
      lowerText.includes("割引") ||
      lowerText.includes("限定") ||
      (lowerText.includes("apple") &&
        !lowerText.includes("注文") &&
        !lowerText.includes("請求")) ||
      subject.includes("広告") ||
    ...
    // カテゴリ分類
    if (
      lowerText.includes("東京電力") ||
      lowerText.includes("関西電力") ||
      lowerText.includes("電気料金") ||
      lowerText.includes("電力")
    ) {
      parsed.category = "electricity";
    } else if (
      lowerText.includes("東京ガス") ||
      lowerText.includes("大阪ガス") ||
      lowerText.includes("ガス料金") ||
      lowerText.includes("ガス")
    ) {
      parsed.category = "gas";
    } else if (
      lowerText.includes("ntt") ||
      lowerText.includes("ドコモ") ||
      lowerText.includes("docomo") ||
      lowerText.includes("携帯") ||
      lowerText.includes("スマホ")
    ) {
      parsed.category = "ntt";
    } else if (
      lowerText.includes("光回線") ||
      lowerText.includes("フレッツ") ||
      lowerText.includes("インターネット") ||
      lowerText.includes("プロバイダ")
    ) {
      parsed.category = "internet";
    } else if (
      lowerText.includes("発送") ||
      lowerText.includes("お届け") ||
      lowerText.includes("配送") ||
      lowerText.includes("注文") ||
      lowerText.includes("amazon") ||
      lowerText.includes("楽天") ||
      lowerText.includes("yahoo") ||
      lowerText.includes("zozo") ||
...
export function calcMonthlyTotal(emails: ParsedEmail[]) {
  const expenseCategories: EmailCategory[] = [
    "electricity",
    "gas",
    "ntt",
    "internet",
